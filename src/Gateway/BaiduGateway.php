...
class BaiduGateway extends GatewayAbstract
{
    use HasHttpRequest;
    ...
    {
        $params = [
            'invokeId' => $this->config->get('sms.gateways.baidu.invoke_id'),
    ...
    {
        return 'http://'.$this->config->get('sms.gateways.baidu.domain', self::ENDPOINT_HOST).self::ENDPOINT_URI;
    }
    ...
    protected function generateSign(array $signHeaders, $datetime)
    {
        ...
        $signingKey = hash_hmac('sha256', $authString, $this->config->get('sms.gateways.baidu.sk'));
        // 生成标准化 URI
